...
### 3.6 Variance Inflation Factor (VIF)
  ...
  ### 3.6 Variance Inflation Factor (VIF) A análise do **Variance Inflation Factor (VIF)** foi utilizada para identificar a presença de multicolinearidade entre as variáveis do dataset, principalmente entre as variáveis numéricas relacionadas às notas dos candidatos. Observou-se que algumas variáveis apresentaram valores extremamente elevados de VIF, indicando forte redundância de informação. Destacam-se a variável **NOTA_CANDIDATO**, com VIF igual a **241,44**, além das variáveis de notas com peso e dos pesos das provas, que também apresentaram valores superiores a 100. Esse resultado indica que essas variáveis carregam informações muito semelhantes, pois a nota final do candidato é construída a partir das notas brutas, dos pesos e das notas ponderadas. Também foram observados valores relevantes de VIF em algumas notas brutas, como **NOTA_R**, com VIF igual a **33,04**, e **NOTA_M**, com VIF igual a **23,38**, indicando que essas variáveis também apresentam relação linear com outras variáveis do conjunto de dados. Por outro lado, variáveis como **NOTA_CORTE**, **CLASSIFICACAO** e **QT_VAGAS_CONCORRENCIA** apresentaram valores menores de VIF, sugerindo menor redundância em relação às demais variáveis numéricas analisadas. É importante destacar que variáveis categóricas, como **SEXO**, **TURNO**, **GRAU**, **SIGLA_IES**, **MUNICIPIO_CAMPUS** e **UF_CANDIDATO**, quando convertidas diretamente para números, não devem ser interpretadas da mesma forma que variáveis numéricas contínuas. Essa conversão pode criar relações artificiais, pois os códigos atribuídos às categorias não representam uma distância real entre elas. Por esse motivo, essas variáveis devem ser analisadas separadamente e, quando utilizadas na modelagem, devem receber tratamento adequado, como **one-hot encoding**. Dessa forma, os resultados indicam que o dataset possui multicolinearidade principalmente devido à presença simultânea de: - notas brutas; - notas ponderadas; - pesos das provas; - nota final do candidato. Como consequência, será necessário realizar uma etapa de seleção de variáveis antes da modelagem, evitando o uso conjunto de variáveis que carregam a mesma informação. Uma alternativa adequada é escolher apenas um grupo de variáveis para o modelo, por exemplo, utilizar as notas brutas e excluir as notas ponderadas, os pesos e a nota final, ou então utilizar a nota final e remover as variáveis que a compõem. Esse tratamento contribui para reduzir a redundância, melhorar a estabilidade dos modelos de machine learning e tornar a análise mais coerente.
